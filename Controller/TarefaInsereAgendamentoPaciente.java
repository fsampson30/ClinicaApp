...
public class TarefaInsereAgendamentoPaciente extends AsyncTask<Agendamento,Void, String> {

    private AgendamentoActivity context;
    private CaminhoServlets caminho = new CaminhoServlets();

    public TarefaInsereAgendamentoPaciente(AgendamentoActivity context) {
    ...
    @Override
    protected String doInBackground(Agendamento... agendamentos) {
        try {
            URLConnection conexao = new URL(caminho.getCaminho()+"/ServletInsereAgendamento").openConnection();
            conexao.setDoOutput(true);
            conexao.setConnectTimeout(10000);
            Agendamento agendamento = agendamentos[0];
            String parametro = URLEncoder.encode(new Gson().toJson(agendamento, Agendamento.class), "UTF-8");
